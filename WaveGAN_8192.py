...
class GanConfig(ModelConfigBase):
    def __init__(self):
        ...
        self.batch_size = 64 
        self.model_size = 64
        self.shift_factor = 2 # Phase Shuffle
...
class Gan(nn.Module):
    def __init__(self, model_config: GanConfig, device):
        ...
        self.critic = Critic(model_config.n_channels, model_config.batch_size,
                                         model_config.model_size, model_config.shift_factor).to(device)
        init_weights(self.critic)
        
        
        self.gen = Generator(model_config.z_dim, model_config.n_channels,
                              model_config.batch_size , model_config.model_size ).to(device)
        init_weights(self.gen)
        
        gen_params, critic_params = get_modules(self)
    ...
    def forward(self, x):
        ...
        noise = 2 * (torch.rand(size=(self.model_config.batch_size, self.model_config.z_dim)).to(self.device)) -1
        fake = self.gen(noise)
    ...
    def train_step(self, real):
        ...
        for _ in range(self.model_config.critic_iterations):
            fake =  self.forward(1) # 1 is dummy input to forego error..
    ...
    def synthesize_data(self):
        ...
        data = torch.permute(self.forward(1).cpu().detach(), (0, 2, 1))
        return data
...
class Generator(nn.Module):
   def __init__(self, z_dim, data_dim, batch_size, model_size, print):
        super(Generator, self).__init__()
        self.net = nn.Sequential(
   
            nn.Linear(z_dim, 256 * model_size), 
...
class GanLossTracker:
    ...
    def update_train_loss(self, n_batches):
        self.loss_critic_history.append(self.sum_loss_d / n_batches)
        self.loss_mse_history.append(self.sum_loss_g / n_batches)
